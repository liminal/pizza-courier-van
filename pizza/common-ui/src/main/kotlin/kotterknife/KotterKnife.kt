...
internal fun <T, V : View> required(id: Int, finder: T.(Int) -> View?)
        = Lazy { t: T, desc -> t.finder(id) as V? ?: viewNotFound(id, desc) }

@Suppress("UNCHECKED_CAST", "UNUSED_ANONYMOUS_PARAMETER")
internal fun <T, V : View> optional(id: Int, finder: T.(Int) -> View?)
        = Lazy { t: T, desc ->  t.finder(id) as V? }

@Suppress("UNCHECKED_CAST")
internal fun <T, V : View> required(ids: IntArray, finder: T.(Int) -> View?)
        = Lazy { t: T, desc -> ids.map { t.finder(it) as V? ?: viewNotFound(it, desc) } }

@Suppress("UNCHECKED_CAST", "UNUSED_ANONYMOUS_PARAMETER")
internal fun <T, V : View> optional(ids: IntArray, finder: T.(Int) -> View?)
...
/*
 * CUSTOM KOTTERKNIFE EXTENSIONS
 */
interface HasViewRoot {
    var rootView: View?
}

fun <V : View> HasViewRoot.bindView(id: Int)
        : ReadOnlyProperty<HasViewRoot, V> = required(id, viewFinder)

fun <V : View> HasViewRoot.bindOptionalView(id: Int)
        : ReadOnlyProperty<HasViewRoot, V?> = optional(id, viewFinder)

fun <V : View> HasViewRoot.bindViews(vararg ids: Int)
        : ReadOnlyProperty<HasViewRoot, List<V>> = required(ids, viewFinder)

fun <V : View> HasViewRoot.bindOptionalViews(vararg ids: Int)
        : ReadOnlyProperty<HasViewRoot, List<V>> = optional(ids, viewFinder)

private val HasViewRoot.viewFinder: HasViewRoot.(Int) -> View?
    get() = { rootView?.findViewById(it) }

private val View.contextFinder: View.() -> Context?
    get() = { rootView?.context }
private val Activity.contextFinder: Activity.() -> Context?
    get() = { this }
private val android.support.v4.app.Fragment.contextFinder: android.support.v4.app.Fragment.() -> Context?
    get() = { context }
private val android.app.Fragment.contextFinder: Fragment.() -> Context?
    get() = { context }
private val HasViewRoot.contextFinder: HasViewRoot.() -> Context?
    get() = { rootView?.context }

fun Activity.bindColor(@ColorRes id: Int)
        : ReadOnlyProperty<Activity, Int> = requiredColor(id, contextFinder)

fun HasViewRoot.bindColor(@ColorRes id: Int)
        : ReadOnlyProperty<HasViewRoot, Int> = requiredColor(id, contextFinder)

fun HasViewRoot.bindDrawable(@DrawableRes id: Int)
        : ReadOnlyProperty<HasViewRoot, Drawable> = requiredDrawable(id, contextFinder)

@Suppress("UNCHECKED_CAST")
internal fun <T> requiredDrawable(id: Int, finder: T.() -> Context?)
        = Lazy { t: T, desc -> ContextCompat.getDrawable((t.finder() ?: viewNotFound(id, desc)), id) as Drawable }

internal fun <T> requiredColor(id: Int, finder: T.() -> Context?)
        = Lazy { t: T, desc -> ContextCompat.getColor((t.finder() ?: viewNotFound(id, desc)), id) }
